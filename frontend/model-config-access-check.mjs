...
assert.doesNotMatch(switcher, /const relayAgent = useMemo/);
assert.match(switcher, /modelLibraryNativeMaintenance/);
assert.match(switcher, /const \[resetAgentId, setResetAgentId\] = useState/);
assert.match(switcher, /api<\{ result: CLIConfigResult \}>\('\/api\/cli-config\/test'/);
assert.match(switcher, /apiKey: apiKey\.trim\(\) \|\| undefined/);
...
assert.match(switcher, /\/api\/cli-config\/presets/);
assert.match(switcher, /const testSavedPreset = async \(preset: CLIConfigPreset\)/);
assert.match(switcher, /onClick=\{\(\) => testSavedPreset\(preset\)\}/);
