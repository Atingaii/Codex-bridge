...
import { AlertCircle, Check, KeyRound, Loader2, Search, ServerCog, Trash2, X } from 'lucide-react';
...
export function CLIConfigSwitcher({ agent, t, close }: { agent: Agent; t: UIText; close: () => void }) {
	...
	const testConnection = async () => {
		if (!capability || (!apiKey.trim() && !testedSecret) || !baseUrl.trim()) return;
		setBusy('test');
		setError('');
		setMessage('');
		try {
			const secret = apiKey.trim() ? await encryptForBridge(apiKey, capability.publicKey) : testedSecret!;
			const data = await api<{ result: CLIConfigResult }>(`/api/agents/${encodeURIComponent(agent.id)}/cli-config/test`, {
				method: 'POST',
				body: JSON.stringify({ cli, baseUrl: baseUrl.trim(), model: model.trim(), secret, keyId: capability.keyId }),
			});
			setBaseUrl(data.result.baseUrl || baseUrl.trim());
			setModels(data.result.models || []);
			setTestedSecret(secret);
			setTested(true);
			setAPIKey('');
	...
	const savePreset = async () => {
		if (!capability || !testedSecret || !tested || !name.trim() || !model.trim()) return;
		setBusy('save');
		setError('');
		try {
			await api(`/api/agents/${encodeURIComponent(agent.id)}/cli-config/presets`, {
				method: 'POST',
				body: JSON.stringify({ cli, name: name.trim(), baseUrl: baseUrl.trim(), model: model.trim(), secret: testedSecret, keyId: capability.keyId }),
			});
			await loadPresets();
			setName('');
			setTested(false);
						...
						<section className="space-y-3 border-t border-border pt-4">
							<div className="grid gap-3 sm:grid-cols-2">
								<Field label={t.providerName}><Input value={name} onChange={(event) => setName(event.target.value)} placeholder={cli === 'codex' ? 'DeepSeek Codex' : 'Claude proxy'} /></Field>
									...
									<Field label={t.apiKey}><Input type="password" autoComplete="new-password" value={apiKey} onChange={(event) => { setAPIKey(event.target.value); setTested(false); setTestedSecret(null); }} placeholder="sk-..." /></Field>
							...
							<div className="flex items-start gap-2 text-[11px] leading-relaxed text-muted-foreground"><KeyRound className="mt-0.5 h-3.5 w-3.5 shrink-0" />{t.apiKeyEncryptedHint}</div>
							<div className="flex flex-wrap gap-2">
									<Button size="sm" variant="secondary" className="gap-1.5" onClick={testConnection} disabled={!!busy || (!apiKey.trim() && !testedSecret) || !baseUrl.trim()}>{busy === 'test' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ServerCog className="h-3.5 w-3.5" />}{busy === 'test' ? t.testingConnection : t.testConnection}</Button>
								<Button size="sm" onClick={savePreset} disabled={!!busy || !tested || !testedSecret || !name.trim() || !model.trim()}>{busy === 'save' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : t.savePreset}</Button>
							</div>
