...
function RunStatsPage({ t, navigate, runId }: { t: UIText; navigate: (path: string) => void; runId: string }) {
  ...
  const costDescription = stats?.costSource === 'provider' ? t.providerCost : stats?.costSource === 'official-catalog' ? t.officialCatalog : stats?.costSource || '';
  const cards = stats ? [
    ...
    { label: t.cacheTokens, value: stats.cacheTokens.toLocaleString(), icon: Layers3 },
    { label: t.totalTokens, value: stats.totalTokens.toLocaleString(), icon: Globe2 },
    { label: t.reasoningTokens, value: (stats.reasoningTokens || 0).toLocaleString(), icon: BarChart3 },
    { label: t.modelCalls, value: (stats.callCount || 0).toLocaleString(), icon: Ticket },
  ...
  return <div className="min-h-screen bg-background text-foreground"><header className="flex min-h-14 items-center justify-between gap-3 border-b border-border px-4 py-2 md:px-8"><div className="flex min-w-0 items-center gap-3"><BarChart3 className="h-5 w-5 shrink-0 text-primary" /><div className="min-w-0"><h1 className="truncate text-sm font-semibold">{t.runStatistics}</h1><p className="truncate text-xs text-muted-foreground">{run?.title || runId}</p></div></div><div className="flex shrink-0 items-center gap-2"><Button variant="ghost" size="icon" onClick={() => void load()} disabled={loading} aria-label={t.refresh} title={t.refresh}><RefreshCw className={loading ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} /></Button><Button variant="secondary" size="sm" onClick={() => navigate('/orchestrate/usage')}><Globe2 className="mr-1.5 h-3.5 w-3.5" />{t.overview}</Button><Button variant="secondary" size="sm" onClick={() => navigate(`/orchestrate/runs/${encodeURIComponent(runId)}`)}><ArrowLeft className="mr-1.5 h-3.5 w-3.5" />{t.backToRun}</Button></div></header><main className="mx-auto max-w-5xl space-y-6 p-4 md:p-8">
    ...
    {loading && !stats ? <div className="flex justify-center p-16"><RefreshCw className="h-5 w-5 animate-spin text-muted-foreground" /></div> : stats ? <>
      <section className="rounded-lg border border-border bg-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase text-muted-foreground">{t.status}</p><p className="mt-1 text-lg font-semibold">{run?.status || t.idle}</p><p className="mt-1 text-xs text-muted-foreground">{stats.accountingSource === 'local-cli-ledger' ? t.localLedgerUsage : t.usageLegacyIncomplete} · {stats.accountingStatus === 'complete' ? t.usageComplete : stats.accountingStatus === 'partial' ? t.usagePartial : t.usageLegacyIncomplete}</p></div><div className="text-right text-xs text-muted-foreground"><div>{stats.startedAt ? formatTime(stats.startedAt) : ''}{stats.finishedAt ? ` -> ${formatTime(stats.finishedAt)}` : ''}</div><div className="mt-1">{stats.native ? t.nativeUsage : t.estimatedUsageNotice}{stats.costKnown ? ` · ${costDescription}` : ` · ${t.costUnavailable}`}</div>{stats.pricingModels?.length ? <div className="mt-1">{t.pricingAnchor}: {stats.pricingModels.join(', ')}</div> : null}<Button className="mt-3" variant="secondary" size="sm" onClick={() => void syncUsage()} disabled={syncing}>{syncing ? t.syncingUsage : t.syncUsage}</Button></div></div></section>
      ...
      <section className="rounded-lg border border-border bg-card p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-semibold">{t.perRound}</h2><p className="mt-1 text-xs text-muted-foreground">{t.inputTokens} / {t.outputTokens} / {t.cacheTokens} / {t.totalTokens}</p></div><BarChart3 className="h-4 w-4 text-muted-foreground" /></div><div className="h-64 w-full">{stats.rounds?.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={stats.rounds} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}><XAxis dataKey="round" tickFormatter={(value) => `#${value}`} /><YAxis tickFormatter={(value) => formatCompact(Number(value))} width={54} /><Tooltip content={<AxisTooltip metric="totalTokens" t={t} />} labelFormatter={(value) => `${t.turns} ${value}`} /><Line type="monotone" dataKey="inputTokens" name={t.inputTokens} stroke="#38bdf8" strokeWidth={2} dot={{ r: 3 }} /><Line type="monotone" dataKey="outputTokens" name={t.outputTokens} stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} /><Line type="monotone" dataKey="cacheTokens" name={t.cacheTokens} stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} /><Line type="monotone" dataKey="totalTokens" name={t.totalTokens} stroke="#a78bfa" strokeWidth={2.5} dot={{ r: 3 }} /></LineChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">{t.noUsageYet}</div>}</div></section>
      <section className="rounded-lg border border-border bg-card"><div className="border-b border-border px-5 py-3 text-sm font-semibold">{t.perCli}</div><div className="divide-y divide-border">{stats.byCli.length ? stats.byCli.map((item) => <div key={`${item.cli}:${item.model}`} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_repeat(5,auto)] sm:items-center"><div><div className="font-medium capitalize">{item.cli}</div><div className="text-xs text-muted-foreground">{item.model || 'default'} · {item.native ? t.nativeUsage : t.unavailableUsage}</div>{item.pricingModel ? <div className="mt-1 text-xs text-muted-foreground">{t.pricingAnchor}: {item.pricingModel}</div> : null}</div>{[[t.modelCalls, item.callCount || 0], [t.inputTokens, item.inputTokens], [t.outputTokens, item.outputTokens], [t.cacheReadTokens, item.cacheReadTokens]].map(([label, value]) => <div key={String(label)} className="text-xs"><span className="text-muted-foreground">{label}</span><div className="font-medium tabular-nums">{Number(value).toLocaleString()}</div></div>)}<div className="text-xs"><span className="text-muted-foreground">{t.estimatedCost}</span><div className="font-medium tabular-nums">{item.costKnown ? `$${item.estimatedCostUsd.toFixed(6)}` : '—'}</div></div></div>) : <div className="p-5 text-sm text-muted-foreground">{t.noUsageYet}</div>}</div></section>
    </> : null}
  </main></div>;
}
