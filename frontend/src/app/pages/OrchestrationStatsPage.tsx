  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-5xl space-y-6 p-4 md:p-8">
      ...
      {!runId ? <div className="rounded-lg border border-dashed border-border p-12 text-center text-sm text-muted-foreground">{t.noUsageYet}</div> : loading && !stats ? <div className="flex justify-center p-16"><RefreshCw className="h-5 w-5 animate-spin text-muted-foreground" /></div> : stats && <>
        <section className="rounded-lg border border-border bg-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-wider text-muted-foreground">{t.status}</p><p className="mt-1 text-lg font-semibold">{run?.status || t.idle}</p></div><div className="text-right text-xs text-muted-foreground"><div>{stats.startedAt ? formatTime(stats.startedAt) : ''}{stats.finishedAt ? ` -> ${formatTime(stats.finishedAt)}` : ''}</div><div className="mt-1">{stats.native ? t.nativeUsage : t.estimatedUsageNotice}{stats.costKnown ? ` · ${costDescription}` : ` · ${t.costUnavailable}`}</div>{stats.pricingModels?.length ? <div className="mt-1">{t.pricingAnchor}: {stats.pricingModels.join(', ')}</div> : null}</div></div></section>
        ...
        <section className="rounded-lg border border-border bg-card"><div className="border-b border-border px-5 py-3 text-sm font-semibold">{t.perCli}</div><div className="divide-y divide-border">{stats.byCli.length ? stats.byCli.map((item) => <div key={`${item.cli}:${item.model}`} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_repeat(4,auto)] sm:items-center"><div><div className="font-medium capitalize">{item.cli}</div><div className="text-xs text-muted-foreground">{item.model || 'default'} · {item.native ? t.nativeUsage : t.unavailableUsage}</div>{item.pricingModel ? <div className="mt-1 text-xs text-muted-foreground">{t.pricingAnchor}: {item.pricingModel}</div> : null}</div><div className="text-xs"><span className="text-muted-foreground">{t.inputTokens}</span><div className="font-medium tabular-nums">{item.inputTokens.toLocaleString()}</div></div><div className="text-xs"><span className="text-muted-foreground">{t.outputTokens}</span><div className="font-medium tabular-nums">{item.outputTokens.toLocaleString()}</div></div><div className="text-xs"><span className="text-muted-foreground">{t.cacheReadTokens}</span><div className="font-medium tabular-nums">{item.cacheReadTokens.toLocaleString()}</div></div><div className="text-xs"><span className="text-muted-foreground">{t.estimatedCost}</span><div className="font-medium tabular-nums">{item.costKnown ? `$${item.estimatedCostUsd.toFixed(6)}` : '—'}</div></div></div>) : <div className="p-5 text-sm text-muted-foreground">{t.noUsageYet}</div>}</div></section>
      </>}
    </main>
  </div>;
}
