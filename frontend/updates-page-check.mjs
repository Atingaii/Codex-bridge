...
assert(updates.includes('当前版本 v0.3.43'), 'missing current version');
...
assert(updates.includes('中间对话栏不再显示重复的左右规划面板'), 'missing duplicate planning rail removal update');
assert(updates.includes('证明路线与运行节点分层展示'), 'missing proof progress hierarchy update');
assert(updates.includes('删除意图立即停止持久化任务图'), 'missing durable delete convergence update');
assert(updates.includes('规划进度内联折叠展示'), 'missing inline planning progress update');
assert(updates.includes('用户选择不再被后台刷新覆盖'), 'missing explicit orchestration selection update');
assert(updates.includes('运行记录支持安全删除'), 'missing orchestration delete state-machine update');
assert(updates.includes('新增双 Agent 规划与审核'), 'missing plan and review agents update');
assert(updates.includes('覆盖式规划进度工作台'), 'missing overlay progress workspace update');
assert(updates.includes('不再猜测安装布局'), 'missing generic tool environment update');
assert(updates.includes('只开放明确工具组件'), 'missing PATH component isolation update');
assert(updates.includes('恢复真实 Home 语义'), 'missing real home environment update');
...
assertCount(updates, "date: '2026", 14);
assertCount(updates, "title: '", 38);
