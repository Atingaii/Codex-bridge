...
func RelayGuidance(userPrompt, mode, role string) string {
	...
	b.WriteString("- Use explicit timeouts with a forced-kill grace for proof-assistant commands, for example `timeout --kill-after=10s 120s make check`. Run proof probes serially; do not leave detached or stale Coq/Isabelle/Lean jobs unless you report their PID/log/exit status in the visible answer.\n")
