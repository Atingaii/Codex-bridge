...
func TestCodexExecStrictWorkspaceUsesOuterSandboxForNewAndResumedSessions(t *testing.T) {
	cfg := config.Default()
	cfg.Bridge.CWD = "/tmp/work"
	cfg.Bridge.Sandbox = "workspace-write"
	cfg.Bridge.ApprovalPolicy = "never"
	cfg.Bridge.StrictWorkspace = true
	runner := NewCodexExecRunner(&cfg)

	for name, args := range map[string][]string{
		"new":    runner.args(RunnerRequest{}),
		"resume": runner.args(RunnerRequest{RemoteThreadID: "thr_strict"}),
	} {
		if !containsArg(args, "--dangerously-bypass-approvals-and-sandbox") {
			t.Fatalf("strict %s args do not bypass nested Codex sandbox: %#v", name, args)
		}
		for _, disallowed := range []string{"--sandbox", `sandbox_mode="workspace-write"`, `approval_policy="never"`} {
			if containsArg(args, disallowed) {
				t.Fatalf("strict %s args include nested policy %q: %#v", name, disallowed, args)
			}
		}
	}
}
